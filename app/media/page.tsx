...
import { mediaTracks, references } from "../lib/content";
...
export default function MediaPage() {
  return (
    <SiteChrome>
      ...
      <section className="track-section section-frame"><div className="section-heading"><Eyebrow number="02">THREE MEDIA TRACKS</Eyebrow><h2>三条内容线，<br /><em>共同指向实践。</em></h2></div><div className="track-grid">{mediaTracks.map((track) => <article className="track-card" id={track.slug} key={track.slug}><span className="media-index">{track.number}</span><span className="track-format">{track.format}</span><h3>{track.title}</h3><p>{track.text}</p><span className="track-mark">● 真实记录</span></article>)}</div></section>
      ...
      <section className="episode-section section-frame"><div className="section-heading"><Eyebrow number="05">EPISODE SEEDS</Eyebrow><h2>可以从这些问题<br /><em>开始说。</em></h2></div><div className="episode-list">{['我为什么不再把 AI 当聊天工具','我如何用 Codex 修改一个真实项目','不会写代码的人应该先学什么','FocusLock 这个产品为什么存在'].map((item, index) => <div key={item}><span>0{index + 1}</span><strong>{item}</strong><span>待录制 →</span></div>)}</div></section>
      <section className="resources section-frame"><Eyebrow number="06">REFERENCE SHELF</Eyebrow><h2>借鉴方法，<br /><em>不复制别人的人生。</em></h2><div className="resource-grid">{references.map((reference) => <a className="resource-card" href={reference.href} target="_blank" rel="noreferrer" key={reference.title}><strong>{reference.title}</strong><span>{reference.note}</span><b>↗</b></a>)}</div></section>
    </SiteChrome>
  );
}
